...
\begin{table}[htpb]
    ...
    \begin{tabularx}{\textwidth}{l l l X}
        ...
        \midrule
        \textbf{Sensor de Flujo} & Rojo (VCC) & VN & Alimentación de señal segura para el micro.\\
...
\begin{itemize}
    \item \textbf{ADC (Conversor Analógico-Digital):} Un "traductor" electrónico. Convierte las señales físicas y continuas (como el sonido del agua variando de volumen) en números precisos que la computadora del ESP32 puede entender y analizar.
    \item \textbf{Alucinación (en IA):} Ocurre cuando la Inteligencia Artificial se "inventa" datos o da respuestas falsas pero mostrándose muy segura de sí misma. En el proyecto ajustamos el modelo de Groq para eliminar este comportamiento y garantizar diagnósticos reales.
    \item \textbf{API (Application Programming Interface):} Un "puente" o mensajero que permite que nuestra aplicación móvil se comunique y pida favores a otros programas en internet (en este caso, pedirle a la IA de Groq que analice los datos).
    \item \textbf{API REST:} El estilo o las "reglas de etiqueta" que usa nuestra aplicación para pedirle información a otros servidores en internet de forma estandarizada y ordenada.
    \item \textbf{Backend:} La parte "trasera" del sistema que el usuario no ve. Se encarga de guardar datos de forma segura, procesar contraseñas y manejar la lógica pesada.
    \item \textbf{Caudalímetro (Sensor de Flujo):} Un dispositivo físico que tiene una pequeña turbina adentro. Cuando el agua pasa, la hace girar y nos permite medir exactamente cuántos litros por minuto están fluyendo por la tubería.
    \item \textbf{CRUD:} Acrónimo de Crear, Leer (Read), Actualizar (Update) y Eliminar (Delete). Son las cuatro acciones básicas que un Administrador puede hacer con el registro de los usuarios en la aplicación.
    \item \textbf{Dart:} El lenguaje de programación que usamos junto con Flutter para darle las instrucciones lógicas a la aplicación móvil.
    \item \textbf{Edge Computing / Edge AI:} Una tecnología que permite que el dispositivo (el sensor bajo el lavaplatos) "piense" y tome decisiones por sí mismo localmente, en lugar de enviar el audio a internet para que un servidor decida. Es como tener un experto vigilando in situ.
    \item \textbf{Efecto Hall:} El principio físico que usa el sensor de agua. Utiliza campos magnéticos para detectar cada vez que la turbina da una vuelta completa, permitiendo medir el agua sin que los circuitos eléctricos se mojen.
    \item \textbf{ESP32:} Una computadora en miniatura, económica pero muy potente, que tiene Wi-Fi integrado. Funciona como el "cerebro" físico que lee el agua y el sonido en la tubería de la casa.
    \item \textbf{Estado (State):} La "memoria a corto plazo" de la aplicación. Por ejemplo, recordar si el usuario ya inició sesión, qué color de tema eligió, o cuál es la lectura actual del sensor de agua en pantalla.
    \item \textbf{Falso Positivo:} Una falsa alarma. Ocurre cuando el sistema dice que hay una fuga, pero en realidad fue un ruido normal de la casa (como lavar platos o encender la licuadora).
    \item \textbf{Flutter:} Herramienta creada por Google que utilizamos para construir la aplicación móvil. Funciona como un kit de piezas de construcción que nos permite hacer la app para Android y iPhone al mismo tiempo usando un solo código.
    \item \textbf{Framework:} Un "esqueleto" o base de trabajo de software que ya viene con muchas herramientas listas para usar, para no tener que programar funciones matemáticas o botones de la aplicación desde cero.
    \item \textbf{Frontend:} La parte "frontal" o visual de la aplicación. Es todo lo que el usuario puede ver y tocar: los botones, colores, textos y animaciones de ondas en la pantalla del celular.
    \item \textbf{Gemelo Digital (Digital Twin):} Una réplica virtual de un aparato físico. Lo usamos en el código para simular cómo se comportarían los sensores de agua y sonido en la vida real, permitiéndonos probar la app exhaustivamente sin tener las tuberías conectadas.
    \item \textbf{GPIO (Pines de Entrada/Salida):} Las "patitas" metálicas del microcontrolador ESP32 donde conectamos los cables de los sensores físicos para que reciban o envíen pulsos eléctricos.
    \item \textbf{Hash / Hashing:} Una técnica de seguridad que toma una contraseña (ej. "1234") y la convierte matemáticamente en un texto incomprensible ("x9f8!z..."). Así, si un intruso roba la base de datos, no podrá leer las claves reales.
    \item \textbf{IoT (Internet de las Cosas):} El concepto de conectar objetos cotidianos (como un refrigerador, un bombillo o, en este caso, una tubería bajo el lavaplatos) a internet para que puedan enviar información y ser monitoreados desde un celular.
    \item \textbf{JSON:} Un formato de texto muy organizado, ligero y con llaves (\{ \}). Es la forma en la que la Inteligencia Artificial nos envía las respuestas de manera que nuestra app pueda leerlas y entenderlas automáticamente.
    \item \textbf{Latencia:} El tiempo de retraso que tarda la información en viajar. Una "baja latencia" en nuestro proyecto significa que el usuario recibe la alerta de fuga en su celular de manera casi instantánea.
    \item \textbf{LLM (Large Language Model):} Un cerebro de Inteligencia Artificial (como ChatGPT o Llama 3) que ha leído cantidades inmensas de texto. Lo usamos para que evalúe los datos del flujo y el sonido, y emita un diagnóstico escrito comprensible para el usuario.
    \item \textbf{Microcontrolador:} Un chip electrónico diseñado para controlar tareas muy específicas. A diferencia de una computadora portátil, está diseñado para no apagarse nunca y conectarse directamente con cables, luces y sensores físicos.
    \item \textbf{Migración de Base de Datos:} El proceso de actualizar la estructura "tabular" donde guardamos los datos sin borrar la información vieja. Lo usamos, por ejemplo, cuando la app evolucionó y tuvimos que agregar la columna "Rol" a los usuarios que ya existían.
    \item \textbf{POJO / Modelo de Datos:} Un "molde" o plantilla en el código que define cómo debe estructurarse la información. Por ejemplo, el modelo "Usuario" dicta que toda persona registrada debe tener obligatoriamente un nombre, un correo y un nivel de acceso.
    \item \textbf{Prompt (Instrucción):} El conjunto de instrucciones exactas y detalladas (ocultas para el usuario) que le enviamos a la Inteligencia Artificial por detrás, indicándole cómo debe comportarse y cómo analizar las lecturas de los sensores.
    \item \textbf{RBAC (Control de Acceso Basado en Roles):} Un sistema de seguridad que funciona como los gafetes de una empresa. Decide qué pantallas o botones puede usar una persona en la app; un "Administrador" puede borrar a otros usuarios, pero un "Operador" solo puede observar los gráficos.
    \item \textbf{Regex (Expresiones Regulares):} Una regla de código que sirve como un filtro estricto de texto. La usamos en el registro para revisar que un correo electrónico realmente tenga la forma de "usuario@correo.com", o que la contraseña cumpla los requisitos de seguridad.
    \item \textbf{Sensor Fusion (Fusión de Sensores):} La técnica estrella del proyecto para la precisión. Consiste en no confiar en un solo sensor (como el micrófono), sino cruzar y comparar matemáticamente los datos de dos sensores diferentes (sonido + flujo de agua) para saber con certeza total si es una fuga real o un uso normal del grifo.
    \item \textbf{Smart Home (Hogar Inteligente):} Un entorno residencial equipado con tecnología, redes y sensores que automatizan tareas rutinarias para mejorar la seguridad, la comodidad y evitar catástrofes como inundaciones.
    \item \textbf{SMTP:} El sistema de mensajería estándar de internet. Es el "cartero" digital que nuestra aplicación utiliza por debajo para enviar correos electrónicos reales al buzón del usuario cuando solicita recuperar su contraseña.
    \item \textbf{SQLite:} Una base de datos pequeña pero muy rápida que vive escondida directamente dentro de la memoria del celular del usuario. Permite que la app guarde los historiales y usuarios sin requerir servidores externos caros.
    \item \textbf{TDD (Desarrollo Guiado por Pruebas):} Una metodología rigurosa de programación donde primero se escriben "exámenes automáticos" y luego se crea el código de la app. Si la app pasa todos los exámenes al compilar, se garantiza que no se romperá por errores imprevistos.
    \item \textbf{Temperatura (en IA):} Un valor de ajuste en la Inteligencia Artificial que controla qué tan "creativa" se pone. En nuestro código la fijamos en "0.0" (cero creatividad) para obligarla a dar respuestas lógicas, robóticas y cien por ciento basadas en los datos del sensor.
    \item \textbf{Token:} Una llave digital, pase de seguridad o cadena de caracteres temporales. Lo usamos para identificarnos de forma segura ante el servicio de Groq, demostrando que tenemos permisos válidos para utilizar su Inteligencia Artificial.
    \item \textbf{UI / UX:} UI (Interfaz de Usuario) es toda la estética visual de la aplicación (formas, sombras, botones). UX (Experiencia de Usuario) es cómo se siente usarla (si la navegación es lógica, si es rápida y si no confunde a quien la opera).
    \item \textbf{VCC y GND:} Los términos técnicos electrónicos universales para la energía. VCC es la "corriente viva" (positivo) que alimenta de electricidad al sensor, y GND es el polo a "Tierra" (negativo) necesario para cerrar cualquier circuito.
    \item \textbf{Widget:} En el lenguaje de Flutter, absolutamente todo elemento visual en la pantalla es un Widget. Un botón es un widget, un campo de texto es un widget, e incluso la pantalla completa es un widget "padre" que agrupa a muchos widgets "hijos".
\end{itemize}
