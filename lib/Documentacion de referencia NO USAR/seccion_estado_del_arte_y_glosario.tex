...
API Key & Clave de acceso a una interfaz de programación. Cadena de texto secreta que
actúa como credencial para autenticar solicitudes a un endpoint protegido. En EdgeLeak~AI
se utiliza el header HTTP \texttt{x-api-key} con el valor configurado en la variable de
entorno \texttt{LOCAL\_API\_KEY} para restringir el acceso al endpoint
\texttt{GET /api/history} a aplicaciones autorizadas.
\\[0.5em]
...
Buffer Circular (Rolling Buffer) & Estructura de datos de tamaño fijo que sobrescribe
los elementos más antiguos cuando se llena. En EdgeLeak~AI, el
\texttt{DashboardController} mantiene un buffer circular de las \textbf{últimas 10
lecturas} del ESP32 (ruido, flujo, estado, timestamp) que permite analizar la tendencia
reciente del sensor sin acumular datos indefinidamente en memoria.
\\[0.5em]
...
Cooldown (Rate Limiting) & Período de espera obligatorio entre dos eventos consecutivos
del mismo tipo. En EdgeLeak~AI, tras cada llamada a la API de Groq se inicia un cooldown
de \textbf{3 minutos} registrado en \texttt{\_lastGroqEvaluation}. Durante ese período,
incluso si el estado ``Posible Fuga'' persiste, no se realizan llamadas adicionales,
protegiendo así la cuota gratuita de la API.
\\[0.5em]
...
dart:io & Biblioteca estándar del lenguaje Dart que provee acceso a operaciones de
entrada/salida del sistema operativo, incluyendo sockets TCP/UDP, acceso al sistema de
archivos y, crucialmente para EdgeLeak~AI, la clase \texttt{HttpServer} que permite
crear un servidor HTTP local sin necesidad de paquetes de terceros.
\\[0.5em]
...
Endpoint & Punto de acceso de una API REST; corresponde a la combinación de método
HTTP (\texttt{GET}, \texttt{POST}, etc.) y ruta URL que identifica de forma única una
operación disponible en el servidor. EdgeLeak~AI expone dos endpoints en el puerto 8080:
\texttt{POST /api/sensor} y \texttt{GET /api/history}.
\\[0.5em]
...
Header HTTP & Metadato enviado en el encabezado de una petición o respuesta HTTP que
proporciona información adicional sobre la transacción, sin formar parte del cuerpo del
mensaje. Ejemplos relevantes en EdgeLeak~AI: \texttt{Content-Type: application/json}
(indica el formato del cuerpo) y \texttt{x-api-key: <clave>} (autenticación del
endpoint \texttt{GET /api/history}).
\\[0.5em]

HTTP (HyperText Transfer Protocol) & Protocolo de comunicación sin estado basado en el
modelo cliente-servidor que define cómo se intercambian mensajes en la web. EdgeLeak~AI
utiliza HTTP/1.1 tanto para la comunicación interna entre el ESP32 y la app (puerto 8080)
como para las llamadas externas a la API REST de Groq.
\\[0.5em]
...
Rate Limiting & Técnica de control de tráfico que restringe el número de operaciones
o peticiones que un sistema puede realizar dentro de un intervalo de tiempo. En
EdgeLeak~AI se aplica a las llamadas a la API de Groq: solo se dispara una evaluación
después de 3 lecturas consecutivas de ``Posible Fuga'' y existe un cooldown de 3 minutos
...
REST (Representational State Transfer) & Estilo arquitectónico para el diseño de servicios
web que utiliza los métodos del protocolo HTTP (\texttt{GET}, \texttt{POST},
\texttt{PUT}, \texttt{DELETE}) para operar sobre recursos identificados por URLs.
La API local de EdgeLeak~AI (\texttt{LocalApiService}) es una implementación RESTful
ligera que expone los recursos \texttt{/api/sensor} y \texttt{/api/history}.
\\[0.5em]
